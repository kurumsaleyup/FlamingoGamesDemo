...
public class Player : MonoBehaviour
{
	...
	private void Start()
	{
		for (int i = 0; i < transform.childCount; i++)//çocuklar ekleniyor
		{
	...
	private void Update()
	{
		...
		else if (Input.GetMouseButton(0))
		{
			...
			if (direction == Vector3.zero)
			{
				...
				direction = Mathf.Abs(direction.x) > Mathf.Abs(direction.y) ? new Vector3(1, 0, 0) : new Vector3(0, 0, 1); //direction belirlendi
			}

			var mouseDistance = mousePos - mouseStartPos;
			if (direction.x != 0 && mousePos.x != mouseStartPos.x)
			{
				var distance = new Vector3(Mathf.RoundToInt(mouseDistance.x * sensitivity), 0, mouseDistance.y); //mouse.x farkları alındı (x de hareket ediyor)
				targetPos = playerStartPos + direction * distance.x; //distance x sayesinde ilerlenecek birim belirleniyor.
			}
			else if (direction.z != 0 && mousePos.y != mouseStartPos.y)
			{
				var distance = new Vector3(mouseDistance.x, 0, Mathf.RoundToInt(mouseDistance.y * sensitivity));//mouse.y farkları alındı (y de hareket ediyor)
	...
	private void FixedUpdate()
	{
		...
		{
			targetPos = new Vector3(Mathf.Clamp(targetPos.x, -12f, 12f), 0, Mathf.Clamp(targetPos.z, -25f, 25f));//posisyon güncellmesi
			rb.MovePosition(Vector3.MoveTowards(playerPos, targetPos, speed * Mathf.Max(minSpeed, (playerPos - targetPos).magnitude) * Time.fixedDeltaTime)); //movement
		}
	}
